...
4. **数字用阿拉伯数字。** 译文中的数字尽量用阿拉伯数字书写: 三到八小时 → 3-8 小时、二十年前 → 20 年前、六千克面粉 → 6000 克面粉、百分之九十五 → 95%、四点四米 → 4.4 米、750 摄氏度 → 750 摄氏度。用于专有名词/标题里的数字(如 "How It's Made"、年份、型号)除外。
5. **注解字幕。** 字幕中出现中文语境少见的当地习俗、节日、专业名词、英制单位等, 添加 ≤20 字(约 4 字/秒)的简短注解, 写入 `main.yaml` 的 `annotations` 列表。**同一术语只在第一次出现时注解。**
...
**校对完成后, 再通扫一遍 `main.yaml`, 把适合阿拉伯数字表示的数字直接用 edit 改成阿拉伯数字**:
- `en`(听写)里的英文词形数字: `three to eight hours` → `3 to 8 hours`、`six thousand kilos` → `6,000 kilos`、`twenty years ago` → `20 years ago`、`half a centimeter` → `0.5 centimeter`、`seventy-five` → `75`、`first`/`second` → `1st`/`2nd`。
- `chs`(译文)里的中文数字: 二十年前 → 20 年前、三到八小时 → 3-8 小时、六千公斤 → 6000 公斤、百分之九十五 → 95%、四点四米 → 4.4 米。
- 用于专有名词/标题/年份/型号的数字保持不变(如 "How It's Made"、`Narrator:` 里的年份)。
- 只改明确无疑的; 拿不准的就保持原文。
